// ============================================================
// AI记忆沙盒 · RPG冒险模式 + 战斗系统 真实AI集成测试
// 调硅基流动 API 跑完整 RPG 模式流程，验证战斗结算走的是程序而不是AI
// ============================================================
...
// ---------- 战斗结算（唯一由程序计算胜负的地方，与 index.html 一致） ----------
...
async function main() {
  console.log('\n═══════════════════════════════════');
  console.log('  AI 记忆沙盒 · RPG冒险模式 + 战斗系统测试');
  ...
  if (createParsed.state) {
    ...
    createParsed.state.世界设定原文 = '这是一场发生在中世纪奇幻大陆的RPG冒险。玩家是刚出师的年轻剑士。';
  ...
  // ---- 第 2 步：主动挑起战斗 ----
  console.log('─── 第 2 步：主动挑起战斗 ───\n');
  const battleMsg = [
    { role: 'system', content: systemPrompt(memory) },
    { role: 'user', content: buildPrompt('我在路上遇到一个拦路的强盗，直接拔剑冲上去和他打起来', memory, history) }
  ];
  console.log('  ⏳ 触发战斗...');
  ...
  if (battleParsed.state) {
    ...
    combatDeclared = !!battleParsed.state.战斗;
    check(combatDeclared, '④ AI 在 STATE 中声明了"战斗"字段（未自行编造胜负）', `实际字段: ${Object.keys(battleParsed.state).join(', ')}`);
    ...
    if (combatDeclared) {
      ...
      // 程序结算（这是本测试的核心验证点：胜负由程序算，不是AI editorializing）
      combatResolveResult = resolveCombat(battleParsed.state.战斗, battleParsed.state.战斗属性, battleParsed.state.元素克制);
      check(combatResolveResult !== null, '⑥ 程序成功结算战斗（resolveCombat 返回结果）');
      if (combatResolveResult) {
        console.log('\n  ' + formatBattleLog(combatResolveResult).split('\n').join('\n  '));
        check(typeof combatResolveResult.winner === 'string' || combatResolveResult.winner === null, '⑦ 结算结果含明确胜方或平局判定');
        check(Object.keys(combatResolveResult.updatedStats).length > 0, '⑧ 结算后回写了双方战斗属性');

        // 模拟 index.html 里 handleUserInput 的第 6.5 步：把结果写回 memory
        ...
        memory.战斗结果 = combatResolveResult.winner
          ...
          : '战斗未分出胜负';
        check(!memory.战斗, '⑨ "战斗"字段结算后已从记忆中移除（不会重复触发）');
      }
    } else {
      updateMemory(memory, battleParsed.state);
    }
  ...
  history.push({ input: '拔剑迎战拦路强盗', narrative: battleParsed.narrative });
  await sleep(500);

  // ---- 第 3 步：验证AI续写战斗后果时尊重程序结果，不重新编排胜负 ----
  if (combatResolveResult) {
    console.log('─── 第 3 步：AI 续写战斗后果（验证不重判过程）───\n');
    const followMsg = [
      { role: 'system', content: systemPrompt(memory) },
      { role: 'user', content: buildPrompt('战斗结束了，看看周围，处理一下战斗的后续', memory, history) }
    ];
    console.log('  ⏳ 续写战斗后果...');
    const followResult = await callAI(followMsg);
    const followParsed = parseAIOutput(followResult);
    if (followParsed.state) updateMemory(memory, followParsed.state);

    const narrativeMentionsResult = combatResolveResult.winner
      ? followParsed.narrative.includes(combatResolveResult.winner) ||
        (combatResolveResult.fallen.length > 0 && combatResolveResult.fallen.some(f => followParsed.narrative.includes(f)))
      : true;
    check(narrativeMentionsResult, '⑩ 续写内容与程序结算结果一致（未凭空改判胜负）',
      `程序结果: ${memory.战斗结果}`);
    console.log(`  📝 ${followParsed.narrative.slice(0, 300)}...\n`);
    history.push({ input: '处理战斗后续', narrative: followParsed.narrative });
  }

  // ---- 第 4 步：非战斗行动（探索/交谈）仍走AI自由发挥 ----
  console.log('─── 第 4 步：非战斗行动（验证RPG模式下探索仍由AI自由叙事）───\n');
  const exploreMsg = [
    { role: 'system', content: systemPrompt(memory) },
    { role: 'user', content: buildPrompt('走进旅店，找老板娘艾米聊聊最近的传闻', memory, history) }
  ];
  console.log('  ⏳ 探索/交谈...');
  const exploreResult = await callAI(exploreMsg);
  const exploreParsed = parseAIOutput(exploreResult);
  if (exploreParsed.state) updateMemory(memory, exploreParsed.state);
  check(!exploreParsed.state || !exploreParsed.state.战斗, '⑪ 非冲突行动不会被误声明为战斗');
  checkContains(exploreParsed.narrative, '艾米', '⑫ 艾米在场景中作为独立NPC回应');
  console.log(`  📝 ${exploreParsed.narrative.slice(0, 300)}...\n`);

  function checkContains(text, keyword, label) {
    check(text.includes(keyword), label, `期望含"${keyword}"`);
  }

  // ---- 结果汇总 ----
  console.log('═══════════════════════════════════');
  console.log('  RPG冒险模式 + 战斗系统测试结果');
  console.log('═══════════════════════════════════');
  console.log(`\n  ✅ 通过: ${passed}`);
  ...
  console.log('─── 最终记忆快照 ───');
  const printKeys = Object.keys(memory).filter(k => k !== '世界设定原文');
  for (const k of printKeys) {
    const v = typeof memory[k] === 'object' ? JSON.stringify(memory[k]).slice(0, 150) : String(memory[k]).slice(0, 150);
    console.log(`  ${k}: ${v}`);
  }
  console.log('');
